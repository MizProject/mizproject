#

![head](mzp.png)

---

# Welcome to the MizProject's GitHub Organization Page

This organization Believes in freedom of customizability and reliability of software and hardware that we commonly use. This group is dedicated to creating and sharing open-source projects that are practical and sets limits beyond the OEM's set standard

## Commitment to the Open-source

MizProject is commited to support Open-Source by Publicly sharing changes and modifications on the work that's under the OSS agenda. 

And also committed to try, learn and adapt new changes of the whole software environment

## Our reach

We were once trying to enable extended support on few Samsung Devices. And we did!

## Contacts

The organization's email can be reached out: [mizproject@proton.me](mailto:mizproject@proton.me)

In telegram: [https://t.me/proj_mizumo_discuss](https://t.me/mizdiscuss)


## Partners

We thank these organizations for their support from the start.

This organization wouldn't be founded without their support at the first place.

Exynos 850: [Telegram Link](https://t.me/ex850chat)
- Head: [@physwizz](https://github.com/physwizz)
- Inspired some of his works: (Kernel, Community, and TWRP port)

Rissu (Primarily their Rissu-KSU): [@rsuntk](https://github.com/rsuntk)

Samsung & Android Tweaks: https://t.me/Samsung_Tweaks
- Head: Ravindu Deshan
- Inspired some of works: Droidspace, Ubuntu-chroot